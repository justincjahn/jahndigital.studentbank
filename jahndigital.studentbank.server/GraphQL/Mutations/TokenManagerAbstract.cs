...
namespace jahndigital.studentbank.server.GraphQL.Mutations
{
    ...
    public class TokenManagerAbstract
    {
        ...
        protected string? GetToken(IHttpContextAccessor context) =>
            context.HttpContext?.Request.Cookies?[TOKEN_COOKIE];
        ...
        {
            var httpc = context.HttpContext
                ?? throw new ArgumentNullException("Unable to fetch HTTP Context object to clear token cookie.");
        ...
        {
            if (context.HttpContext?.Request.Headers.ContainsKey("X-Forwarded-For") ?? throw new Exception("Unable to fetch HttpContext.")) {

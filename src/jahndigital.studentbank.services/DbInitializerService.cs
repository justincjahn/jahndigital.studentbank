...
namespace jahndigital.studentbank.services
{
    ...
    public class DbInitializerService : IDbInitializerService
    {
        ...
        private void SeedPrivileges(AppDbContext context)
        {
            ...
            foreach (Constants.Privilege? privilege in Constants.Privilege.Privileges)
            {
        ...
        private void SeedRoles(AppDbContext context)
        {
            ...
            foreach (Constants.Role? role in Constants.Role.Roles)
            {
                if (dbRoles.Where(x => x.Name == role.Name).FirstOrDefault() == null)
                {
                    ...
                    foreach (Constants.Privilege? privilege in role.Privileges)
                    {
                        Privilege? priv = dbPrivileges.Where(x => x.Name == privilege.Name).First();
        ...
        {
            Product? extraCredit = _productsCache.Where(x => x.IsLimitedQuantity = true).FirstOrDefault();
        ...
        {
            Product? chocolateBar = _productsCache.Where(x => x.IsLimitedQuantity).FirstOrDefault();
